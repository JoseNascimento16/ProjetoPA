...
class EventGroup(object):
    ...
    def _log_event(self, type, message, initiator=None, plano_base=0, plano_situacao_log='', send_mail=None):
        """
        Log a new event entry.
        """
        EventModel = apps.get_model('eventlog', 'Event')
        event_object = EventModel.objects.create(
            type=type, group=self.group_id, message=message, initiator=initiator, plano_base=plano_base, plano_situacao_log=plano_situacao_log)
    ...
    def _send_mail(self, email, event_object):
        ...
        context = {
            ...
            'plano_base':event_object.plano_base,
            'plano_situacao_log':event_object.plano_situacao_log,
